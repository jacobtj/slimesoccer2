...
class Player(pygame.sprite.Sprite):
    ...
    def reset(self):
        ...
        if self.player_number == 1:
            ...
            self.rect.center = (WIDTH - 100, HEIGHT)
    ...
    def update(self, collided_players, player1, player2):
        ...
        if collided_players:
            ...
            if player.vel_x != 0 or player.vel_y != 0:
                self.vel_x = self.vel_x + player.vel_x * 0.5
...
menu_options = ['Resume Game', 'Restart Game', 'Quit']
...
def render_menu():
    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            # check if the mouse is within the bounds of each menu button
            for i, option in enumerate(menu_options):
                text_surface = font.render(option, True, BLACK)
                text_rect = text_surface.get_rect()
                text_rect.center = (WIDTH / 2, HEIGHT / 2 + i * 50)
                if text_rect.collidepoint(mouse_pos):
                    # perform an action based on which button is clicked
...
while running:
    ...
    score_text = font.render(str(player1.score) + " - " + str(player2.score), True, WHITE)
